...
class ReplyPostAPI(HttpUser):
    """Test POST /posts/{postId}/replies API
    ...
    The number of posts in the pool can be configured via the REPLY_POST_POOL_SIZE
    ...
    def reply_post(self):
        ...
        self.client.post(
            ...
            json={
                "userId": self.user_id,
                "content": f"Reply load test {random_string(20)}",
            },
        )
...
def on_test_start(environment, **kwargs):
    ...
    pool_size = int(os.getenv("REPLY_POST_POOL_SIZE", "10"))
    ...
    for i in range(pool_size):
        body = PostPostsBody(
            user_id=user_id, content=f"Shared post for reply load test {i + 1}/{pool_size} {random_string(10)}"
        )
        post_response = post_posts.sync(client=client, body=body)
        ReplyPostAPI.post_pool.append(str(post_response.post_id))
